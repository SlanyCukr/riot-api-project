...
class DetectionResponse(BaseModel):
    ...
    analysis_time_seconds: Optional[float] = Field(
        None, description="Analysis time in seconds"
    )
...
class DetectionStatsResponse(BaseModel):
    ...
    smurf_detection_rate: float = Field(
        ..., description="Percentage of analyses that detected smurfs"
    )
    average_score: float = Field(..., description="Average detection score")
    confidence_distribution: Dict[str, int] = Field(
        ..., description="Distribution of confidence levels"
    )
    factor_trigger_rates: Dict[str, float] = Field(
        ..., description="Rate at which each factor triggers detection"
    )
    queue_type_distribution: Dict[str, int] = Field(
        ..., description="Distribution by queue type"
    )
...
class BulkDetectionRequest(BaseModel):
    ...
    max_concurrent: int = Field(
        5, ge=1, le=10, description="Maximum concurrent analyses"
    )
...
class DetectionTrendAnalysis(BaseModel):
    ...
    factor_changes: Dict[str, float] = Field(
        ..., description="Changes in individual factors"
    )
...
class DetailedDetectionResponse(DetectionResponse):
    ...
    signals: List[DetectionSignalAnalysis] = Field(
        ..., description="Detailed signal analysis"
    )
    recommendations: List[DetectionRecommendation] = Field(
        ..., description="Detection recommendations"
    )
    trend_analysis: Optional[DetectionTrendAnalysis] = Field(
...
class DetectionCalibrationRequest(BaseModel):
    ...
    known_legitimate: List[str] = Field(
        ..., description="List of known legitimate players"
    )
...
class DetectionCalibrationResponse(BaseModel):
    ...
    optimized_thresholds: Dict[str, float] = Field(
        ..., description="Optimized thresholds"
    )
...
class DetectionAlertResponse(BaseModel):
    ...
    last_triggered: Optional[datetime] = Field(
        None, description="When alert was last triggered"
    )

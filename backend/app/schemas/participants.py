...
class ParticipantSearchRequest(BaseModel):
    ...
    match_id: Optional[str] = Field(
        None, max_length=64, description="Filter by match ID"
    )
    ...
    individual_position: Optional[str] = Field(
        None, max_length=16, description="Filter by position"
    )
...
class MatchParticipantsBulkCreate(BaseModel):
    ...
    participants: list[MatchParticipantCreate] = Field(
        ..., min_length=1, max_length=10, description="List of participants"
    )

...
class MatchParticipantRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def find_by_player_in_matches(
        self, puuid: str, match_ids: list[str]
    ) -> list[MatchParticipantORM]:
        """Get all participant records for a player across multiple matches.
        ...
        Returns:
            List of participant records (one per match where player participated)
        """
        pass
...
class SQLAlchemyMatchParticipantRepository(MatchParticipantRepositoryInterface):
    ...
    async def find_by_player_in_matches(
        ...
        Single query instead of N queries for N matches.
        """
        if not match_ids:
            return []
        ...
        stmt = select(MatchParticipantORM).where(
            and_(
                MatchParticipantORM.puuid == puuid,
                MatchParticipantORM.match_id.in_(match_ids),
            )
        )
        ...
        logger.debug(
            "participants_found_for_player_in_matches",
            puuid=puuid,
            match_count=len(match_ids),
            found_count=len(participants),
        )

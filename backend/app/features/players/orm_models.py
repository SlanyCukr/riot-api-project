...
class PlayerORM(Base):
    ...
    def win_rate(self) -> float:
        """Get the highest win rate from current ranks.

        :returns: Win rate as percentage (0-100) or 0.0 if no ranks
        """
        if not self.ranks:
            return 0.0
        current_ranks = [r for r in self.ranks if r.is_current]
        if not current_ranks:
            return 0.0
        return max(r.win_rate for r in current_ranks)

    @property
    def total_games(self) -> int:
        """Get total games played across all current ranked queues.

        :returns: Sum of games across all current ranks
        """
        if not self.ranks:
            return 0
        current_ranks = [r for r in self.ranks if r.is_current]
        if not current_ranks:
            return 0
        return sum(r.total_games for r in current_ranks)

    @property
    def display_rank(self) -> str:
        """Get display rank from Solo/Duo queue.

        :returns: Formatted rank string (e.g., 'Gold II') or 'Unranked'
        """
        if not self.ranks:
            return "Unranked"
        solo_rank = next(
            (r for r in self.ranks if r.is_current and r.queue_type == "RANKED_SOLO_5x5"),
            None
        )
        return solo_rank.display_rank if solo_rank else "Unranked"

    def is_high_elo(self) -> bool:
        """Check if player is high elo in any current queue.

        :returns: True if Diamond or above in any current queue
        """
        if not self.ranks:
            return False
        return any(r.is_high_elo() for r in self.ranks if r.is_current)
...
class PlayerRankORM(Base):
    ...
    def is_provisional(self) -> bool:
        """Check if rank is provisional (less than 10 games).

        :returns: True if fewer than 10 games played
        """
        return self.total_games < 10

    def is_fresh(self) -> bool:
        """Check if rank data is fresh (updated within 24 hours).
    ...
    def calculate_mmr_estimate(self) -> float:
        """Estimate MMR based on tier, rank, and LP.

        Business calculation for approximate matchmaking rating.

        :returns: Estimated MMR value
        """
        tier_base = {
            "IRON": 0,
            "BRONZE": 400,
        ...
        base = tier_base.get(self.tier.upper(), 0)
        division = rank_value.get(self.rank, 0) if self.rank else 0

        return float(base + division + self.league_points)

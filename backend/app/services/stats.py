...
class StatsService:
    ...
    async def calculate_player_statistics(
        ...
        try:
            ...
            logger.error(
                "Failed to calculate player statistics", puuid=puuid, error=str(e)
            )
            raise
    ...
    async def calculate_match_statistics(self, match_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_damage": sum(
                    p.total_damage_dealt_to_champions for p in participants
                ),
            }
        except Exception as e:
            logger.error(
                "Failed to calculate match statistics", match_id=match_id, error=str(e)
            )
            raise

    async def calculate_encounter_statistics(
        self, puuid: str, limit: int = 50, min_encounters: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for other_puuid, data in encounter_data.items():
                ...
                if total_encounters >= min_encounters:
                    # Calculate win rates
                    teammate_wins = sum(
                        1 for m in data["matches"] if m["is_teammate"] and m["win"]
                    )
                    opponent_wins = sum(
            ...
            logger.error(
                "Failed to calculate encounter statistics", puuid=puuid, error=str(e)
            )
            raise
    ...
    async def _calculate_champion_stats(
        self, participants: List[MatchParticipant]
    ) -> Dict[str, Any]:
    ...
    async def _calculate_performance_trends(
        ...
        sorted_participants = sorted(
            participants, key=lambda p: p.match.game_creation, reverse=True
        )
    ...
    def _calculate_team_match_stats(
        self, team_participants: List[MatchParticipant]
    ) -> Dict[str, Any]:

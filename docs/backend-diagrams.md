...
erDiagram
    ...
    PLAYERS ||--o{ SMURF_DETECTIONS : "has detections"
    MATCHES ||--o{ MATCH_PARTICIPANTS : "contains"

    PLAYERS {
        string puuid PK "Riot PUUID"
        string riot_id "name#tag"
        string summoner_name "Legacy name"
        string platform "Region"
        int account_level "Level"
        timestamp last_seen "Last activity"
    }

    MATCHES {
        string match_id PK "Match ID"
        bigint game_creation "Timestamp"
        int queue_id "420=Ranked"
        int game_duration "Seconds"
        bool is_processed "Analyzed"
    }

    MATCH_PARTICIPANTS {
        bigint id PK "Auto-increment"
        string match_id FK "Match"
        string puuid FK "Player"
        int champion_id "Champion"
        int kills "K"
        int deaths "D"
        int assists "A"
        bool win "Won"
    ...
    PLAYER_RANKS {
        ...
        string puuid FK "Player"
        string tier "GOLD, DIAMOND"
        string rank "I, II, III, IV"
        int league_points "LP"
        bool is_current "Latest rank"
    }

    SMURF_DETECTIONS {
        int id PK "Auto-increment"
        string puuid FK "Player"
        bool is_smurf "Detected"
        decimal smurf_score "0.0-1.0"
        string confidence "high/medium/low"
        int games_analyzed "Sample size"
        timestamp last_analysis "Analysis time"
    }

    DATA_TRACKING {
        int id PK "Auto-increment"
        string data_type "account/match/rank"
        string identifier "PUUID/Match ID"
        timestamp last_fetched "API fetch time"
        int fetch_count "API calls"
    }

    API_REQUEST_QUEUE {
        int id PK "Auto-increment"
        string data_type "Request type"
        string priority "urgent/high/normal"
        string status "pending/processing"
        timestamp scheduled_at "Queue time"
    }

    RATE_LIMIT_LOG {
        int id PK "Auto-increment"
        string endpoint "API endpoint"
        int limit_count "Limit value"
        int current_usage "Usage"
...
```mermaid
---
title: FastAPI Endpoints Structure
---
graph TB
    ROOT[FastAPI Application<br/>/api/v1]

    ROOT --> PLAYERS[/players<br/>Player Management]
    ROOT --> MATCHES[/matches<br/>Match Data]
    ROOT --> DETECT[/detection<br/>Smurf Analysis]
    ROOT --> HEALTH[/health<br/>System Status]

    PLAYERS --> P1[GET /search<br/>Search by Riot ID/Name]
    PLAYERS --> P2[GET /:puuid<br/>Get by PUUID]
    PLAYERS --> P3[POST /bulk<br/>Bulk fetch players]
    PLAYERS --> P4[GET /:puuid/recent<br/>Recent opponents]

    MATCHES --> M1[GET /:puuid/history<br/>Match history]
    MATCHES --> M2[GET /:match_id<br/>Match details]
    MATCHES --> M3[POST /refresh<br/>Refresh matches]

    DETECT --> D1[POST /analyze<br/>Analyze player]
    DETECT --> D2[GET /:puuid/history<br/>Detection history]
    DETECT --> D3[GET /stats<br/>Detection statistics]
    DETECT --> D4[GET /config<br/>Algorithm config]
    DETECT --> D5[POST /bulk<br/>Bulk analysis]

    HEALTH --> H1[GET /<br/>Health check]
    HEALTH --> H2[GET /cache-stats<br/>Cache statistics]

    style ROOT fill:#6C757D,stroke:#333,color:#fff
    style PLAYERS fill:#0D6EFD,stroke:#333,color:#fff
    style MATCHES fill:#17A2B8,stroke:#333,color:#fff
    style DETECT fill:#DC3545,stroke:#333,color:#fff
    style HEALTH fill:#28A745,stroke:#333,color:#fff
```
...
graph TB
    subgraph "Docker Compose Services"
        subgraph "Backend Container"
            FASTAPI[FastAPI Application<br/>Port 8000<br/>Python 3.13 + uv]
            ALEMBIC[Alembic Migrations<br/>Auto-run on startup]
            FASTAPI --> ALEMBIC
        end

        subgraph "Database Container"
            POSTGRES[PostgreSQL 18<br/>Port 5432<br/>Persistent volume]
        end

        subgraph "Frontend Container"
            REACT[React + Vite<br/>Port 5173<br/>Hot reload enabled]
        end
    end

    subgraph "Volumes"
        DB_VOL[postgres_data<br/>Database persistence]
        BE_VOL[./backend<br/>Code mount]
        FE_VOL[./frontend<br/>Code mount]
    end

    subgraph "Networks"
        NET[riot-api-network<br/>Internal communication]
    end

    FASTAPI -.->|Volume mount| BE_VOL
    POSTGRES -.->|Volume mount| DB_VOL
    REACT -.->|Volume mount| FE_VOL

    FASTAPI <-->|SQLAlchemy| POSTGRES
    REACT <-->|API calls| FASTAPI

    FASTAPI -.->|Bridge network| NET
    POSTGRES -.->|Bridge network| NET
    REACT -.->|Bridge network| NET

    EXTERNAL[External Client] -->|localhost:5173| REACT
    EXTERNAL -->|localhost:8000| FASTAPI

    style FASTAPI fill:#6C757D,stroke:#333,color:#fff
    style POSTGRES fill:#198754,stroke:#333,color:#fff
    style REACT fill:#0D6EFD,stroke:#333,color:#fff
    style EXTERNAL fill:#FFC107,stroke:#333,color:#000
```

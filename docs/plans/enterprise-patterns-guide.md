...
├── test_orm_models.py         # Domain logic tests
├── test_repository.py         # Data access tests
├── test_service.py            # Orchestration tests (mocked)
├── test_integration.py        # Full-stack tests
└── factories.py               # Test data factories
```

### Template: Domain Model Tests

```python
# tests/features/<feature>/test_orm_models.py
"""Tests for domain model business logic."""

import pytest
from app.features.<feature>.orm_models import <Entity>ORM


def test_business_calculation():
    """Test domain calculation logic."""
    entity = <Entity>ORM(field1=100, field2=50)
    result = entity.business_calculation()
    assert result == 150.0


def test_business_rule_true():
    """Test business rule when condition met."""
    entity = <Entity>ORM(status="active")
    assert entity.business_rule() is True


def test_business_rule_false():
    """Test business rule when condition not met."""
    entity = <Entity>ORM(status="inactive")
    assert entity.business_rule() is False


def test_validation_success():
    """Test validation passes for valid entity."""
    entity = <Entity>ORM(name="Valid Name", count=10)
...
def test_validation_failure():
    ...
    entity = <Entity>ORM(name="", count=-1)  # Invalid
    errors = entity.validate()
    assert len(errors) > 0
...
```python
# tests/features/<feature>/test_repository.py
"""Tests for repository data access layer."""

import pytest
from sqlalchemy.ext.asyncio import AsyncSession
from app.features.<feature>.orm_models import <Entity>ORM
from app.features.<feature>.repository import SQLAlchemy<Entity>Repository
...
async def test_get_by_id_found(db: AsyncSession):
    ...
    # Arrange
    entity = <Entity>ORM(name="Test Entity")
    db.add(entity)
    await db.commit()
    await db.refresh(entity)

    repo = SQLAlchemy<Entity>Repository(db)
...
async def test_get_by_id_not_found(db: AsyncSession):
    ...
    # Act
    result = await repo.get_by_id(99999)  # Non-existent ID
...
async def test_create(db: AsyncSession):
    """Test create persists new entity."""
    # Arrange
    repo = SQLAlchemy<Entity>Repository(db)
    entity = <Entity>ORM(name="New Entity")
    ...
    # Verify in database
    result = await db.get(<Entity>ORM, created.id)
    assert result is not None


@pytest.mark.asyncio
async def test_find_by_criteria(db: AsyncSession):
    """Test find_by_criteria filters correctly."""
    # Arrange
    entity1 = <Entity>ORM(name="Active", is_active=True)
    entity2 = <Entity>ORM(name="Inactive", is_active=False)
    ...
    # Act
    results = await repo.find_by_criteria(is_active=True)
...
### Template: Service Tests (with Mocks)
...
from app.features.<feature>.orm_models import <Entity>ORM
from app.features.<feature>.schemas import <Entity>Create
from app.features.<feature>.exceptions import <Entity>NotFoundError, ValidationError
...
@pytest.mark.asyncio
async def test_get_entity_success(mock_repository):
    """Test get_entity returns entity when found."""
    # Arrange
    entity_orm = <Entity>ORM(id=1, name="Test", is_active=True)
    mock_repository.get_by_id = AsyncMock(return_value=entity_orm)

    service = <Entity>Service(mock_repository)

    # Act
    result = await service.get_entity(1)
...
@pytest.mark.asyncio
async def test_get_entity_not_found(mock_repository):
    """Test get_entity raises exception when not found."""
    # Arrange
    ...
    with pytest.raises(<Entity>NotFoundError):
        await service.get_entity(99999)


@pytest.mark.asyncio
async def test_create_entity_success(mock_repository):
    """Test create_entity creates valid entity."""
    # Arrange
    entity_orm = <Entity>ORM(id=1, name="New Entity", is_active=True)
    mock_repository.create = AsyncMock(return_value=entity_orm)

    service = <Entity>Service(mock_repository)
    entity_data = <Entity>Create(name="New Entity")
...
@pytest.mark.asyncio
async def test_create_entity_validation_error(mock_repository):
    """Test create_entity fails validation."""
    # Arrange
    service = <Entity>Service(mock_repository)
    entity_data = <Entity>Create(name="")  # Invalid
...
```python
# tests/features/<feature>/test_integration.py
...
@pytest.mark.asyncio
async def test_get_entity_endpoint(client: AsyncClient, db: AsyncSession):
    """Test GET /<entities>/{id} endpoint."""
    # Arrange: Create entity in database
    entity = <Entity>ORM(name="Test Entity", is_active=True)
    db.add(entity)
    await db.commit()
    await db.refresh(entity)
...
@pytest.mark.asyncio
async def test_create_entity_endpoint(client: AsyncClient):
    """Test POST /<entities> endpoint."""
    # Arrange: Prepare request body
    payload = {"name": "New Entity", "description": "Test description"}
...
@pytest.mark.asyncio
async def test_get_nonexistent_entity_endpoint(client: AsyncClient):
    """Test GET /<entities>/{id} with nonexistent ID."""
    # Act: Request nonexistent entity
    response = await client.get("/api/v1/<entities>/99999")
